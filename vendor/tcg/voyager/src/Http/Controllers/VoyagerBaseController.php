        ...
        if($slug=='orders')
        {
            $order=Order::find($id);
                ...
                if($request->approved == 1){
                    ...
                    $message='عزيزي العميل لقد تم استلام المبلغ وعليه نفيدكم بانه تم اعتماد حفلكم الكريم برقم العقد ('.$order->code_number.') نامل الدخول على موقع مارينا الغربية وطباعة العقد. وشكراً لا اختياركم موقع مارينا لخدمة كبار الشخصيات.';
                    
                    
                    
                        //$message='لقد تم استلام المبلغ و تأكيد حفلكم الكريم نأمل الدخول علي الموقع وطباعة العقد الخاص بكم رقم ('.$order->code_number.') وشكرا لاخيتاركم مارينا الغربية لخدمة كبار الشخصيات.';
            ...
            if($request->delied==1)
            {
                ...
                $message='
عزيزي العميل تمت الموافقه على تأجيل الحفل الخاص بكم لرقم العقد  ';
             $message.=$order->code_number;
                $mobile = User::find($order->user_id)->phone;
                send_sms($message,$mobile);

            }
            if($request->canceled==1)
            {
            ...
            if($request->closed==1)
            {
                ...
                $message= "عزيزي العميل نفيد سعادتكم بأنه تم إغلاق العقد رقم    ".$order->code_number;
                     $message.='  شاكرين ومقدرين حسن تعاملك موقع مارينا الغربية لخدمة كبار الشخصيات';
...

        {
            ...
            $order=Order::find($groom->order_id);

            if($groom->approved==2&&$request->approved==1){
                $message='تمت الموافقه ع طلب اضافه العريس الخاص بكم للعقد رقم ';
                $message.=$order->code_number;
                $mobile = User::find($order->user_id)->phone;
                send_sms($message,$mobile);
