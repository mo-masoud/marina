...
<div class="form-style">
    <form action="{{ url('singer/step3') }}" method="POST">
    	{{ csrf_field() }}
        <div class="col-md-12 col-xs-12 wow animation-div ">
        	<div class="form-group">
            	<p>
                	 {{ trans('front.machine_choose') }}  :
                </p>
            </div>
        </div>
        ...
        @foreach(Machines() as $Machine)
	        ...
	        <div class="col-md-6 col-xs-12 wow animation-div ">
	        	<div class="form-group otherStyle">
	            	<label class="checkB">
	            		{{ $Machine->getTranslatedAttribute('name', session('lang') ) }}
	                  <input
                            type="checkbox"
                            {{ ( machine_session($Machine->id) == $Machine->getTranslatedAttribute('name', session('lang') )  ) ? 'checked' : '' }}
                            name="machine[{{ $Machine->id }}]"
                            value="{{ $Machine->getTranslatedAttribute('name', session('lang') ) }}"
                            />
