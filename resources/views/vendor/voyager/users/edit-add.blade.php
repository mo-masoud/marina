            ...
            <div class="row">
                    ...
                    {{-- <div class="panel"> --}}
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                ...
                                $options = $row->details;
                                @endphp
                                <label for="gender">{{ __('voyager::generic.gender') }}</label>
                                <select class="form-control select2" id="gender" name="gender">
                                    <option value="" disabled>اختر النوع</option>

                                    @foreach ($row->details->options as $key => $value)
                                    <option value="{{ $key }}"
                                    {{ ($key == $dataTypeContent->gender ? 'selected' : '') }}>{{ $value }}</option>
                                    @endforeach
                                </select>    
                            </div>
                            ...
                            <div class="form-group">
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $row     = $dataTypeRows->where('field', 'approved')->first();
                                $options = $row->details;
                                @endphp
                                <label for="approved">تفعيل الحساب</label>
                                <select class="form-control select2" id="approved" name="approved">
                                    @foreach ($row->details->options as $key => $value)
                                    <option value="{{ $key }}"
                                    {{ ($key == $dataTypeContent->approved ? 'selected' : '') }}>{{ $value }}</option>
                                    @endforeach
                                </select>    
                            </div>
                            ...
                            @can('editRoles', $dataTypeContent)
                                ...
                                @if(auth()->user()->hasRole('singer'))
                                    <strong>This User Will be Your Modrator</strong>
                                    <input type="hidden" value="5" name="role_id"> <!-- 5 is the modrator -->
                                @else
                                    <div class="form-group">
                ...
                <div class="col-md-4">
                    ...
                    <div class="panel panel panel-bordered panel-warning">
                        <div class="panel-body">
                            <div class="form-group">
                                <label for="logo">شعار المطرب</label>
                                ...
                                @endif
                                <input type="file" data-name="logo" name="logo" id="logo">
                            </div>
                        </div>
                    </div>
                </div>
