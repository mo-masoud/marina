...
@section('content')
                    ...
                    @foreach($dataType->readRows as $row)
                       ...
                       @if($dataTypeContent->{$row->field} != null)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @elseif($row->type == 'hejry')

                                @php
                                    $hegry = new \DateTime($dataTypeContent->{$row->field});
                                    if (session('lang') == 'ar'){
                                        $format = 'ar_SA';
                                    }else{
                                            $format = 'en_US';
                            ...
                            @elseif($row->type == 'array')
                               ...
                               @if(is_array($dataTypeContent->{$row->field}))
                                @foreach($dataTypeContent->{$row->field} as $key => $data)
    ...
    <div class="panel-heading" style="border-bottom:0;">
                            <h3 class="panel-title"> الجنسية </h3>
                        </div>
                        <div class="panel-body" style="padding-top:0;">
                            {{ (\App\Models\Order::find(Request::segment(3)))->ordernationality ? (\App\Models\Order::find(Request::segment(3)))->ordernationality->name : null}}
