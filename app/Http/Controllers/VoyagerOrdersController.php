...
class VoyagerOrdersController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
               ...
               // $query = $model::select('*');
                $query = $model::where("provided_by_marina",1);
              if( isset(request()->query()['new_date'])=='new_date')
                {
                  //  $query = $model::select('*');
                    $query = $model::where("provided_by_marina",1);

                    $panned_orders = \App\Models\Order::where('delied', 2)->orWhere('delied',1)->get()->pluck('id');
                    $query = $query->whereIn('id', $panned_orders);
                ...
                else
                {
                    if (auth()->user()->hasRole('singer')) {
                        ...
                        //$panned_orders = \App\Models\Order::where([['closed', 0], ['canceled', 0]])->orwhere([['closed', null], ['canceled', null]])->get()->pluck('id');

                        $panned_orders=$query->get()->pluck("id");
                        $query = $query->whereIn('id', $panned_orders);
    ...
    public function closed(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
                ...
                $query = $model::select('*');
                $panned_orders = \App\Models\Order::where('closed','<>',0)->get()->pluck('id');
                $query = $query->whereIn('id',$panned_orders);
    ...
    public function canceled(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
                ...
                $query = $model::select('*');
                $panned_orders = \App\Models\Order::where('canceled','<>',0)->get()->pluck('id');
                $query = $query->whereIn('id',$panned_orders);
	...
	public function print_order($code_number) {
		$order = Order::where('id', $code_number)->first();
